...
function validateUserInsert(user) {
	...
	const username =
			...
			isLength(user.username, { min: 5, max: 200 }),
		password =
			user.password &&
			!isEmpty(user.password) &&
			isLength(user.password, { min: 8 }) &&
			matches(user.password, passwordRegex),
		firstName =
			user.firstName &&
			!isEmpty(user.firstName) &&
			isLength(user.firstName, { min: 2, max: 50 }),
		lastName =
			user.lastName &&
			!isEmpty(user.lastName) &&
			isLength(user.lastName, { min: 2, max: 50 }),
		type = user.type && isLength(user.type, { min: 3, max: 10 }),
		company = user.company && isLength(user.company, { min: 3, max: 10 });

	//required fields
	if (!username) {
		errors.push('Username is empty or invalid');
	}
	...
	if (!user.terms) {
		errors.push(
			'You have to accept our terms of service in order to register.'
		);
	}
...
function validatePasswordReset(newPassword, newPasswordConf) {
	...
	password =
		newPassword &&
		!isEmpty(newPassword) &&
		isLength(newPassword, { min: 8 }) &&
		matches(newPassword, passwordRegex);
	passwordConf =
		newPasswordConf &&
		!isEmpty(newPasswordConf) &&
		matches(newPasswordConf, newPassword);
